...
def validate(model, criterion, dataloader, logger, device=None):
    model.eval()

    epoch_losses = []

    tqdm_iter = tqdm.tqdm(enumerate(dataloader), total=len(dataloader))
    with torch.no_grad():
        for i, batch in tqdm_iter:
            imgs, true_masks = batch["image"], batch["mask"].float()
            masks_pred = model(imgs.to(device)).float()
    ...
    logger.info(f"Validation finished! Loss: {mean_loss:.5f}")
...
def main(args):
    ...
    val_dataset = DetectionDataset(args.data_path, os.path.join(args.data_path, "train_segmentation.json"),
                                   transforms=train_transforms, split="val")
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_model_info["val_loss"]:
            ...
            logger.info(f"Validation loss: {val_loss:.3f} (best)")
        else:
            logger.info(f"Train loss: {train_loss:.5f} (best {best_model_info['train_loss']:.5f})")
            logger.info(f"Validation loss: {val_loss:.5f} (best {best_model_info['val_loss']:.5f})")
