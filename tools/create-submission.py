...
def parse_arguments():
    ...
    parser.add_argument("-z", "--image_size", dest="image_size", default=256, type=int, help="image size")
...
def main(args):
    ...
    for i, file_name in enumerate(tqdm.tqdm(files)):
        ...
        # 1. Segmentation.
        image, (kx, ky) = prepare_for_segmentation(
            image_src.astype(np.float32) / 255., (args.image_size, args.image_size)
        )
        x = torch.from_numpy(image.transpose(2, 0, 1)).float().unsqueeze(0)
        transform_normalization = transforms.Compose([
            transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.25, 0.25, 0.25))
        ])
        x = transform_normalization(x)
        with torch.no_grad():
