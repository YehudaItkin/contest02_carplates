...
def validate(model, criterion, dataloader, logger, device):
    model.eval()

    epoch_losses = []
    tqdm_iter = tqdm.tqdm(dataloader)
...
def train(model, criterion, optimizer, train_dataloader, logger, device):
    ...
    for i, batch in enumerate(tqdm_iter):
        ...
        # print(f"seqs_pred: {seqs_pred.shape}, \n"
        #       f"log_probs: {log_probs.shape}, \n"
        #       f"seqs: {seqs.shape}, \n"
        #       f"seq_lens_pred: {seq_lens_pred.shape}, \n"
        #       f"seq_lens: {seq_lens.shape} \n")
...
def main(args):
    ...
    val_dataset = RecognitionDataset(args.data_path, os.path.join(args.data_path, "train_recognition.json"),
                                     abc=abc, transforms=train_transforms, split="val")
